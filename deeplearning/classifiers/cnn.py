...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        pad = (filter_size - 1) // 2
        H_conv = 1 + (H + 2 * pad - filter_size)
        W_conv = 1 + (W + 2 * pad - filter_size)
    ...
    def loss(self, X, y=None):
        ...
        # Backward pass
        drelu_out, dW3, db3 = affine_backward(dscores, affine2_cache)
        daffine1_out = relu_backward(drelu_out, relu_cache)
        dpool_out, dW2, db2 = affine_backward(daffine1_out, affine1_cache)
        dconv_out = max_pool_backward_fast(dpool_out, pool_cache)
